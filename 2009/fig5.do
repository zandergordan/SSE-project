...
** NOTE: 133,031 workers are in the cities with population between 1M and 2.5M,
** they have missing values for the last 2 variables

* number of workers and CBSA in sample
sum wage
codebook metarea
...
* save graph
graph export "results/figure_5_left.pdf", replace
graph export "results/figure_5_left.eps", replace
...
twoway 
    ...
    ||
    line y2 x2, lpattern(solid) lcolor(black) legend(label(2 "> `lgstr'"))
	, xtitle("skill (log utility)") ytitle("pdf") 
	scheme(s1color) legend(region(lstyle(none)))
	xsize(5.50) ysize(4.28) scale(1)
	note(
	"10th percentile: pop < `scstr' = `pctile10_group1', pop > `lgstr' = `pctile10_group2', diff = `pctile10_diff'`pctile10_star' (`pctile10_se')" 
...
* save graph
graph export "results/figure_5_right.pdf", replace
graph export "results/figure_5_right.eps", replace
